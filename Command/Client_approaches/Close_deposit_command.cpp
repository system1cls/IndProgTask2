...
Close_deposit_command::Close_deposit_command(Date* date, Time* time, string name, Client_type cl_type, ulong acc_ID) : Command(date, time){
	this->name = name;
	this->cl_type = cl_type;
	this->acc_ID = acc_ID;
}

void Close_deposit_command::run(bank_info& info) {
	if (info.clients_by_name_type.contains(name)) {
		Client* client = info.clients_by_name_type[name];
		string opearation = "close deposit";
		info.analyzer.addNewActivity(client->get_id(), date, time, opearation);
		info.analyzer.add_new_client_enter(date, name);
	}

	if (info.is_open)
		close_deposit(info, date, time, name, cl_type, acc_ID);
}

bool Close_deposit_command::check_parse(string str) {
	return check_string_by_scheme(str, "Close Account");
}

Command* Close_deposit_command::parse(string str) {
	string temp = str;
	...
	Command *cmd = new Close_deposit_command(date, time, name_str, cl_type, acc_id);
	return cmd;
}

bool Close_deposit_command::set_worker(bank_info& info) {
	Work_place* place;
	...
	ulong time_to_be_done = place->add_new_client(this->time, 25);
