...
Withdraw_command::Withdraw_command(Date* date, Time* time, string name, Client_type cl_type, ulong acc_ID, Money* money) : Command(date, time) {
	this->name = name;
...
void Withdraw_command::run(bank_info& info) {
	if (info.clients_by_name_type.contains(name)) {
		Client* client = info.clients_by_name_type[name];
		string opearation = "withdraw";
		info.analyzer.addNewActivity(client->get_id(), date, time, opearation);
		info.analyzer.add_new_client_enter(date, name);
	}

	if (info.is_open)
		withdraw(info, date, time, name, cl_type, acc_ID, money);
}

bool Withdraw_command::check_parse(string str) {
	return check_string_by_scheme(str, "Withdraw Funds");
}

Command* Withdraw_command::parse(string str) {
	string temp = str;
	...
	Command* command = new Withdraw_command(date, time, name_str, cl_type, acc_id, money);
	return command;
}

bool Withdraw_command::set_worker(bank_info& info) {
	Work_place* place;
